...
## Migration record
- 2026-07-09 · tool v1.0.1 (liborrery migration, D-020): declared hash **unchanged** `6c596a53…` (reproduced bit-identical 3×). The envelope's non-hashed `version` field now prints `1.0.1`; `stdout.txt` remains the v1.0.0 freeze-time capture (the gate is `declared.hash`, D-013).
