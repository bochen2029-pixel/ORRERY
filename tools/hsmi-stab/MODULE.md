...
- **P7 winding/index witness, iteration 1 (post-Q-001-ruling; site-basis center-row symbol of `M(t)=P_N U(t) P_N`):** NEGATIVE — chiral winding 0 at every t with a clean gap (gap 0.998→0.156 as t→0.2 at n=128; the flow DISPERSES in site space, no transport band within |d|≤16); t-inv control shows winding only where the gap has collapsed (+2 at gap 0.04, −1 at gap 0.10 — same sign at ±t, junk on a near-origin curve). Note: complex-symmetry of U (T-invariance) constrains the exact symbol to φ(θ)=φ(−θ) ⇒ EVEN winding only; odd control readings are finite-size/noise artifacts — a usable diagnostic.
- **P8 ladder diagnostic (Q-001 iteration 2a):** NEGATIVE — the 𝒜/𝒩 modular ladders ALIGN increasingly with n (mean λ-spread of the overlap 1.04→0.35 chiral, 0.67→0.22 t-inv) but with ZERO directional displacement: δ̄ ≈ 0, frac(δ_j>0) ≈ 0.50, both models, both n. (The displacement scalar provably collapses toward the P6 generic object; the band picture is the informative part.)
- **P9 Wiesbrock-cocycle eigenphase flow (Q-001 iteration 2b; `V(t)=U_𝒩(t)U_𝒜(−t)`, hsm ⟺ one-signed phases):** NEGATIVE, with the controls validating the machinery exactly — **t-inv control neg≡pos to 5 digits, drift ±0.0000** (the named theorem: staggering×conjugation maps V→conj(V) unitarily since S·k·S=−k for both half-filled k's); **chiral: nneg≈n/2, sided 1.06→1.04→1.03 SHRINKING with n**; the whole asymmetry is the trace drift (`arg det V = t(tr k_𝒩 − tr k_𝒜)`), flipping exactly with t; commutant variant (⊕−k_out) indistinguishable; **random nested control symmetric (1.00–1.01)** with a ~5× larger phase band — the cut's phases are symmetric UV-scale content, not inclusion structure. **UV/IR sector diagnostic:** phase sign UNCORRELATED with modular-UV weight (qneg≈qpos); no hidden one-sided IR sector; the n=32 random control shows a SPURIOUS IR-sector sided=6.9 that vanishes at n=128 — a live proof that unregistered sector-mining manufactures false arrows. Eigenphase extraction certified (min|v†Vv| = 1.000000 throughout).

**VERDICT (2026-07-10, closing the Q-001 iterations):** both ruled-in witness forms are measured negatives on a machinery whose controls behave exactly as their theorems require. Across P2–P9 every chirality-sensitive signal that survives the exact blindness identities is a trace/free-energy-scale scalar that decays or saturates with n — nothing shows the hsm signature (one-sidedness persisting or strengthening with n). Escalated as **Q-002** (QUESTIONS.md): continue mining within the ruling vs science-handback of the negative-result chain vs pivot.

**Q-001 RULED (operator, 2026-07-10): index/winding witness adopted (transport cross-check secondary). Next iterations, pre-analyzed:** (i) read the winding in the **rapidity / modular-ladder basis** — the true flow translates the Eisler–Peschel/rapidity coordinate, not the site coordinate, so the site-basis symbol disperses by construction; build the ladder basis from k_A's eigenvectors ordered by eigenvalue and read the shift structure of the compression there; (ii) the **Wiesbrock cocycle** `V(t) = U_𝒩(t)·U_𝒜(−t)` (embed U_𝒩⊕1): for a true hsm `V(t)=e^{2πitG}` with G ≥ 0 ⇒ ALL eigenphases flow one way; V(−t) ≠ V(t)† (operator order!) so no blindness identity applies; witness = the signed eigenphase-flow asymmetry, integer-quantized by winding of det on the ladder; (iii) clamp-hardening for both (the γ=1e−12 clamp caps |λ| at 27.6 and may distort the ladder tails).
