...
| **orrery** | Python | **DONE v1.0.0** (golden `43977185`; det. 3×; the ergonomic CLI over the catalogue, D-033 / TinyUniverse R-1+R-2+R-3 — `list`/`describe`/`run`/`sweep` + one-shot receipt-`verify` (MATCH/MISMATCH exit 0/1) + `mcp-register`; reuses the `mcp` primitives, I-12 chain inherited; cold two-pass CONFORMANT 11/11) | `tools/orrery/` |
