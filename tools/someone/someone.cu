// someone.cu — ORRERY tool `someone` (v1.1.1)
// Headless, deterministic evolutionary Someone-Criterion instrument.
// Contract: contracts/someone.contract.md v1.1.x (+ someone.schema.json). The contract is
...
//
// v1.1.1 [BEHAVIOR-NEUTRAL, D-020]: someone was the TEMPLATE the lib was extracted from —
// this migration deletes its local copies (RNG kit, blake2b, serializers, reductions, spine)
// in favor of lib/, whose KAT selftest pins the extraction verbatim. Golden aa5b731d unchanged.
//
// Build (from tools/someone/, see BUILD.md):
//   cmd /c '"...\vcvars64.bat" >nul 2>&1 && nvcc -O3 -arch=sm_89 someone.cu ../../lib/envelope.cpp -o someone.exe'
...
#include "../../lib/rng.cuh"      // the D-012 kit: splitmix64/hash4/u01/counter_*, h_u01/h_normal
#include "../../lib/reduce.cuh"   // blockReduceSum / blockReduceSum3 (fixed-order, deterministic)
...
// (BLAKE2b + canonical serialization helpers now come from lib/envelope.h — D-020.)
...
// (blockReduceSum / blockReduceSum3 now come from lib/reduce.cuh — the same fixed-order
//  warp-shuffle reductions, extracted verbatim. D-020.)
...
// (h_u01 / h_normal now come from lib/rng.cuh — the host mt19937_64 idiom, extracted verbatim.)
...
static std::string full_envelope(const Params& P, const Result& R, const std::string& verdict){
    return orrery::full_envelope("someone", SOMEONE_VERSION, declared_body(P,R,verdict), FIREWALL);
}
...
// ------------------------------------------------------------------ CLI (die2/parse_ll/parse_d from lib)
