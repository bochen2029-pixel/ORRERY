...
**FOUR tools built this session** (someone, ratchet, posit, mcts) — all golden-frozen and green through the polyglot harness. someone + ratchet + posit are **independently cold-two-pass verified (CONFORMANT)**; **mcts's cold two-pass is in flight** (the only owed item). The instrument spans CUDA + Python and four throats/engines (the gap; the ratchet threshold `(1−p)ρ=p`; the parsimony criterion Q3; a generic UCT search engine). The harness demonstrably catches real defects (it caught ratchet's build-command wiring). Verdicts: `runs/{someone,ratchet,posit,mcts}_twopass_verify.md`.

## Next concrete action (for the next session)
1. Confirm mcts's cold two-pass verdict → drop its caveat.
2. Next tools, copying the template: `algebra` (cuSOLVER crossed-product entropy, feeds F16/D-CP — the last two-pass-critical citable tool), `autotune` (parameter sweep / basin finder over the existing tools). Each: contract-first → golden → cold two-pass.
