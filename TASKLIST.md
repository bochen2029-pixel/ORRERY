...
## Phase 1 — `someone` to full standard (the template)  ← START HERE
- [NOT_STARTED] S1 · Read `contracts/someone.contract.md` + the prototype `C:\Users\user\Desktop\DSA\dak_evolution_complex.cu`. Write `tools/someone/MODULE.md`.
- [NOT_STARTED] S2 · Implement `tools/someone/someone.cu` to the contract: CLI parse (all flags), the evolutionary sim (port dak kernels), `--ensemble`, `--complexity` L0–L3, deterministic reductions (NO atomics in the declared-output reduction path), the JSON envelope, `--csv`, gates (G-ZOMBIE-WINS, G-NO-GAP), `--selftest`, `--golden`.
