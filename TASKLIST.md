...
- [DONE*] `posit` (Python port of posit_counter) — parsimony auditor, the Python-is-right tool (D-005/D-016). Contract v1.0.0 + schema + MODULE + `posit.py` + golden `7a22dd22` (3× byte-identical, exact/no-RNG determinism). selftest green (12 checks: seed-cluster +0.8 win, confabulation guard, floating→G-FLOATING, no-parsimony→G-NO-PARSIMONY, reach guard, determinism). Reads audit cases via `--case`/`--stdin`. `harness/verify.py` made polyglot → GREEN on posit. *(cold two-pass owed; D-POSIT-AGG multi-cluster aggregation deferred to v1.1.0.)*
