...
- [DONE*] `mcts` — generic CUDA root-parallel UCT search engine (D-017). Contract v1.0.0 + schema + MODULE + `mcts.cu` + golden `6c596a53` (3× byte-identical). One tree/thread, index-based node pool, counter-RNG rollouts, deterministic UCB1. Golden: all 1024 trees find the exact optimum of the 4^6 `match` landscape (best_reward=1.0). selftest green (7 checks incl. best_path==target, G-SUBOPTIMAL on starved budget). *(cold two-pass owed; deceptive landscape / custom reward deferred to v1.1.0.)*
