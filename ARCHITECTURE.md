...
| **orrery** | Python | The ergonomic **CLI over the catalogue** (TinyUniverse R-1/R-2/R-3): `list`/`describe`/`run`/`sweep` + one-shot receipt-`verify` + `mcp-register`; reuses the `mcp` primitives so the I-12 hash chain is inherited | new (D-033) | posit-chain self-check (narrow coupling like `mcp`; re-baselines with posit) | **DONE v1.0.0** (golden `43977185`, det. 3×; the R-3 verifier works both ways; cold two-pass CONFORMANT 11/11) |
