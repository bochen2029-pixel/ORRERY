...
| **lens** | CUDA/OptiX | RT-core silhouette render + oracle-gated projected cross-section of physics geometry (orthographic; `sphere` πR², `bhshadow` Schwarzschild capture 27π M² at b_crit=√27 M); honestly scoped per D-004 (render only) | `RAYFORMER/render.cu` (OptiX pipeline) | exact analytic cross-section (πR² / 27π M²); RT↔baseline I-13 paired-oracle | **DONE v1.0.0** (golden `11e545b8`, det. 3× on sm_89+OptiX 9.1.0+drv 610.47; compute-SPIKE pre-registered with a ≥1.5× kill; cold two-pass CONFORMANT) |
