...
| **carve** | C++ (host, fp64) | Preferred-factorization basin search (order-book **Layer-2/P2**): does a fixed H prefer a tensor-product structure? scores a frame by **k-locality Pauli-weight concentration** reported as a **gap over the analytic Haar baseline** `B(N,k)`; greedy discrete-frame descent; gates G-NO-BASIN / G-MULTI-BASIN. §III-sealed. **Design selected by the `runs/carve_design/` Intercom tournament (D-034) — which caught a circular oracle pre-contract** | new (D-026; quantum-mereology, CPR "Locality from the Spectrum" + Carroll–Singh) | planted-scrambler KNOWN answer (on-lattice entangling V; metamorphic un-scramble `oracle_dev`<1e-9, I-11) + analytic Haar baseline; `haar` random control → G-NO-BASIN | **DONE v1.0.0** (golden `1373454e` = ising positive control, det. 3×; selftest 12/12 incl. the three-control gauntlet + anti-blindness; **cold two-pass CONFORMANT 2/2 — autonomous N-way via Intercom, 0 defects**; greedy search-recovery honestly scoped → v1.1 mcts/continuous optimizer) |
